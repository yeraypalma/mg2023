/*
  ...
  Modified to load texture objects also
*/
...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].texturemap = NULL;
		model->materials[i].specularmap = NULL;
		model->materials[i].bumpmap = NULL;
	...
	while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'd':		// PRL 20030806: dissolve factor, pseudo-transparency.
			...
			break;
		case 'm':
			if (strstr(buf, "map_Kd")) {
				fgets(buf, sizeof(buf), file);		// Read up to (and including) EOL from file into string.
				l = strlen(buf);
				buf[l-1] = '\0';								  // nuke '\n'.
				if ((l-2 >= 0) && buf[l-2] == '\r') buf[l-2] = '\0';  // nuke MSDOS lf
				model->materials[nummaterials].texturemap = strdup(buf+1);	// +1 skips leading space.
			} else if (strstr(buf, "map_Ks")) {
				fgets(buf, sizeof(buf), file);		// Read up to (and including) EOL from file into string.
				l = strlen(buf);
				buf[l-1] = '\0';								  // nuke '\n'.
				if ((l-2 >= 0) && buf[l-2] == '\r') buf[l-2] = '\0';  // nuke MSDOS lf
				model->materials[nummaterials].specularmap = strdup(buf+1);	// +1 skips leading space.
			} else if (strstr(buf, "map_Bump")) {
				fgets(buf, sizeof(buf), file);		// Read up to (and including) EOL from file into string.
				l = strlen(buf);
				buf[l-1] = '\0';								  // nuke '\n'.
				if ((l-2 >= 0) && buf[l-2] == '\r') buf[l-2] = '\0';  // nuke MSDOS lf
				model->materials[nummaterials].bumpmap = strdup(buf+1);	// +1 skips leading space.
			}
			else {
	...
	while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'v':               /* v, vn, vt */
			...
			case 't':           /* texcoord */
				...
				//texcoords[2 * numtexcoords + 1] = 1.0 - texcoords[2 * numtexcoords + 1]; // flip V
				numtexcoords++;
				break;
			}
			break;
